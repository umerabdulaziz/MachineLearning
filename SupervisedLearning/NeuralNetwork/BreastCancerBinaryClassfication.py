...
for label in [0, 1]:
    plt.scatter(
        ...
        label=f"Correct {data.target_names[label]}",
        alpha=0.6,
    )
    plt.scatter(
        X_pca[(y.flatten() == label) & (yhat_all.flatten() != label), 0],
        X_pca[(y.flatten() == label) & (yhat_all.flatten() != label), 1],
        label=f"Misclassified {data.target_names[label]}",
        marker='x',
        color='red'
    )

plt.title("Manual Neural Network Classification (Breast Cancer)")
